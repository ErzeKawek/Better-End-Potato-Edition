...
public class BiomeMap 
{
	...
	public BetterEndBiome getBiome(int x, int z)
	{
		...
		if (biome.hasEdge() || (biome.hasParentBiome() && biome.getParentBiome().hasEdge()))
		{
			...
			boolean edge = search.isNotSame(getRawBiome(x + d, z));
			edge = edge || search.isNotSame(getRawBiome(x - d, z));
			edge = edge || search.isNotSame(getRawBiome(x, z + d));
			edge = edge || search.isNotSame(getRawBiome(x, z - d));
			edge = edge || search.isNotSame(getRawBiome(x - 1, z - 1));
			edge = edge || search.isNotSame(getRawBiome(x - 1, z + 1));
			edge = edge || search.isNotSame(getRawBiome(x + 1, z - 1));
			edge = edge || search.isNotSame(getRawBiome(x + 1, z + 1));
